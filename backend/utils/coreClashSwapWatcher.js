...
async function refreshPrices(pair, coreIsToken0, coreDecimals) {
  ...
  // CORE's price comes straight from the CORE/WETN pool this watcher is already watching (reserves x WETN's
  // USD price) — RPC reads only, no ElectroSwap calls. It used to ask ElectroSwap's single-token endpoint
  // (50 credits) every refresh; this is the original method and reads the same pool's reserves.
  try {
