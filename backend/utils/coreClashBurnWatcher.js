...
import { RPC_URL, EXPLORER_BASE_URL, CORE_TOKEN_ADDRESS, POLL_INTERVAL_MS, LOOKBACK_BLOCKS } from "./coreClashConfig.js";
...
async function poll(token, symbol, decimals) {
  if (isPolling) return;
  isPolling = true;
...
export async function startCoreClashBurnWatcher() {
  ...
  poll(token, symbol, decimals);
  setInterval(() => poll(token, symbol, decimals), POLL_INTERVAL_MS);
}
