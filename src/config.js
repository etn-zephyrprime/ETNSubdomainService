...
// wallet would just get a revert. Must be kept in sync if ownership is ever transferred
// (transferOwnership) or the contract is redeployed.
export const MARKETPLACE_OWNER_ADDRESS = "0x3Fd2e5B4AC0efF6DFDF2446abddAB3f66B425099";
// Every deprecated marketplace this app used to point at, most-recent-first — each is left live
// on-chain, no longer written to, but its totalCoreBurned/sellerAmount/listings/activation history
// is real lifetime activity that shouldn't vanish from the site just because the current contract
// is a fresh deployment starting its own on-chain counters at 0. Everything that needs "lifetime,
// not just since the latest switch" (useBurnPool.js's Total CORE Burned, the marketplace listings
// merge, activated-domains/owned-names discovery, migrateActivation's own chain, the various
// backend caches) iterates this list alongside MARKETPLACE_ADDRESS. migrateActivation itself only
// ever reads ONE hop back (its own immutable legacyMarketplace, i.e. LEGACY_MARKETPLACES[0]) — a
// domain activated on V3 but never migrated to V4 needs two calls to reach V5 (V4's own
// migrateActivation first, then V5's), not one; see useSubnamePricing.js's isDomainActivated/
// migrateActivation for how the frontend walks this chain and offers both steps as one button.
// Earlier V2 deployments (0xd9BC87b41c8011c9CaEeda91167cacfFD91Cd22c block 15204649;
